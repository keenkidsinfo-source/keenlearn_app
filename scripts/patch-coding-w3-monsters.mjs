...
const updates = [
  {
    ...
    steps: [
      ...
      '🔄 Make Caterpie come BACK! Still on CATERPIE — after "hide" add 3 more blocks in this order: 1️⃣ Click orange CONTROL → drag "wait 1 seconds" → snap it after "hide" (Caterpie waits invisibly). 2️⃣ Click blue MOTION → drag "go to x: 0 y: 130" → snap it after the wait → change the numbers so it says x: 0 and y: 130 (Caterpie secretly moves to the top while hiding!). 3️⃣ Click purple LOOKS → drag "show" → snap it LAST (now Caterpie pops up right where the ball will go!). Your full "if" block order: change Score → hide → wait → go to x:0 y:130 → show. Press Space and throw — catch it again and again! 🎉',
