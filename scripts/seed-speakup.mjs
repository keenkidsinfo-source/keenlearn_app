...
// ── WEEK 1 ──────────────────────────────────────────────────────────────────
{
  ...
  sessionPlan: [
    { startMin:0, endMin:8, label:'WARM-UP', emoji:'🎭', title:'Name + Feeling Toss',
      steps:[
        {time:'0:00',action:'Welcome! "Today is Day 1 of SpeakUp! You are all speakers NOW." High energy, big smile.'},
        {time:'1:00',action:'Introduce 3 Pillars: hold up 3 fingers. "VOICE (throat) — BODY (arms wide) — MIND (tap head)." Class repeats.'},
        {time:'3:00',action:'Demo Name + Feeling Toss yourself. Mime the invisible ball toss clearly.'},
        {time:'4:30',action:'Students take turns. Prompt shy students: "Happy? Scared? Proud? Pick one!"'},
        {time:'7:00',action:'Round 2 with gesture. Cheer for creative choices. Transition: "Let\'s speak!"'},
      ]},
    { startMin:8, endMin:52, label:'MAIN ACTIVITY', emoji:'🎤', title:'Introduction Speeches',
      steps:[
        {time:'8:00',action:'Word of Day: SPEAKER. "Say it with me: A speaker SHARES ideas!"'},
        {time:'10:00',action:'Model the prompt yourself — keep it real and a little funny.'},
        {time:'12:00',action:'Show the 3-part structure. "I hold up a finger for each part as you go."'},
        {time:'13:00',action:'2-min prep: students whisper-practice with a partner or think quietly.'},
        {time:'15:00',action:'First speaker up. "Feet apart, look at three different people." Start with a volunteer.'},
        {time:'16:10',action:'After each speech: one classmate gives ONE star ("I liked when you said...")'},
        {time:'17:00',action:'Continue through all students. Target 2 min per student (1 min speech + 1 min feedback).'},
      ]},
    { startMin:52, endMin:60, label:'WRAP-UP', emoji:'🌟', title:'Reflection + Send-Off',
      steps:[
        {time:'52:00',action:'"What are the Three Pillars?" Class shouts: VOICE — BODY — MIND!'},
        {time:'54:00',action:'Star of the Day: name one student who was brave today. Be specific.'},
        {time:'55:00',action:'Preview: "Next week: VOLUME — how to fill the whole room with your voice!"'},
        {time:'57:00',action:'Closing ritual: "Repeat after me — I AM A SPEAKER!" Class shouts it. Fist bump.'},
      ]},
  ],
  pictureCards:[
...
// ── WEEK 2 ──────────────────────────────────────────────────────────────────
{
  ...
  sessionPlan: [
    { startMin:0, endMin:8, label:'WARM-UP', emoji:'🎭', title:'Volume Machine',
      steps:[
        {time:'0:00',action:'Show Voice Dial card. "Today\'s Pillar is VOICE. Specifically: VOLUME."'},
        {time:'1:30',action:'Run Volume Machine — call levels 1,5,3,2,5,1 in fast succession.'},
        {time:'5:00',action:'Silent round: hold up fingers only. Class reads and responds.'},
        {time:'7:00',action:'Land on level 3. "This is our classroom voice. Remember it!"'},
      ]},
    { startMin:8, endMin:52, label:'MAIN ACTIVITY', emoji:'🎤', title:'Animal Speeches',
      steps:[
        {time:'8:00',action:'Word of Day: VOLUME. Ask: "What volume should we use in this room?" (level 3-4)'},
        {time:'10:00',action:'Model the animal speech at level 2 (too quiet). Ask: "Could you hear me?" Then redo at level 4.'},
        {time:'13:00',action:'Show structure card (4 parts). 2-min prep. Students may draw their animal.'},
        {time:'15:00',action:'Speeches begin. Hold up Voice Dial if volume drops — signal silently.'},
        {time:'16:30',action:'After each: audience rates the volume 1-5 with fingers.'},
      ]},
    { startMin:52, endMin:60, label:'WRAP-UP', emoji:'🌟', title:'Volume Check-Out',
      steps:[
        {time:'52:00',action:'"What level should you speak at for 14 people? For 100 people?" Quick class poll.'},
        {time:'54:00',action:'Star of Day: student who had the best level 4 volume.'},
        {time:'55:30',action:'Preview: "Next week: PACE — how fast or slow you speak."'},
        {time:'57:00',action:'Closing: "I AM A SPEAKER!" at level 4. Then whisper it at level 1.'},
      ]},
  ],
  pictureCards:[
...
const SESSIONS_G34 = [
{
  ...
  sessionPlan: [
    { startMin:0, endMin:8, label:'WARM-UP', emoji:'🎭', title:'Impromptu Introductions',
      steps:[
        {time:'0:00',action:'Welcome. "No training yet. I want to see where everyone starts." High expectations, warm tone.'},
        {time:'1:00',action:'Impromptu Introductions: 30 seconds each, strict timing. Teacher goes first.'},
        {time:'6:00',action:'Introduce the Three Pillars — ask students to vote which they think is hardest.'},
        {time:'7:30',action:'"By week 14 you will have mastered all three. Let\'s build from today."'},
      ]},
    { startMin:8, endMin:52, label:'MAIN ACTIVITY', emoji:'🎤', title:'Great Communicator Speeches',
      steps:[
        {time:'8:00',action:'Word of Day: RHETORIC. Brief context: Aristotle identified these techniques 2400 years ago.'},
        {time:'9:30',action:'Model the speech structure — point to each section. Demo a 90-second version.'},
        {time:'12:00',action:'5-min prep. Students can use their notes.'},
        {time:'17:00',action:'Speeches begin. After each: one classmate names one specific technique they noticed.'},
        {time:'20:00',action:'Teacher tracks: note each student\'s natural strength (hook boldness, eye contact, structure, etc.)'},
      ]},
    { startMin:52, endMin:60, label:'WRAP-UP', emoji:'🌟', title:'Goal Setting',
      steps:[
        {time:'52:00',action:'"Write ONE speaking goal for this semester. One thing you want to be able to do by December."'},
        {time:'54:00',action:'3-4 students share goals. Teacher affirms each one specifically.'},
        {time:'56:00',action:'Preview: "Next week: VOICE MASTERY — volume, pace, clarity, expression. Your instrument."'},
        {time:'58:00',action:'Closing: "Repeat after me — I AM A SPEAKER." Then: "And this semester, I\'ll prove it."'},
      ]},
  ],
  pictureCards:[
...
},
{
  ...
  sessionPlan: [
    { startMin:0, endMin:8, label:'WARM-UP', emoji:'🎭', title:'Voice Dimension Drill',
      steps:[
        {time:'0:00',action:'"VOICE pillar. Today: your voice has 4 dials — volume, pace, clarity, expression."'},
        {time:'1:00',action:'Run all 4 stations of Voice Dimension Drill (90 sec each). Keep energy high.'},
        {time:'7:00',action:'Students name their weakest dimension. Write it on a sticky note — their focus for today.'},
      ]},
    { startMin:8, endMin:52, label:'MAIN ACTIVITY', emoji:'🎤', title:'Greatest Invention Speeches',
      steps:[
        {time:'8:00',action:'Word of Day: ARTICULATION. Demo muddled vs. crisp delivery of same sentence.'},
        {time:'9:30',action:'Model speech with marked vocal variation — point out where you changed dimension.'},
        {time:'12:00',action:'Prep: students mark their script/notes: V (volume change), P (pace change), E (expression).'},
        {time:'15:00',action:'Speeches begin. Audience tracks: raise hand when they notice a deliberate voice change.'},
        {time:'17:00',action:'Feedback focus: name the specific voice dimension that was strongest.'},
      ]},
    { startMin:52, endMin:60, label:'WRAP-UP', emoji:'🌟', title:'Voice Audit',
      steps:[
        {time:'52:00',action:'Quick self-assessment: each student rates themselves 1-5 on each of the 4 dimensions.'},
        {time:'55:00',action:'Star of Day: most dynamic vocal range.'},
        {time:'56:30',action:'Preview: "Next week: BODY LANGUAGE — the other 55% of your message."'},
        {time:'58:00',action:'Closing ritual with maximum vocal variation — do the send-off in 3 different ways.'},
      ]},
  ],
  pictureCards:[
