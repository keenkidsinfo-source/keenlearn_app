...
const PREP_INFO: Record<string, { image: string; steps: string[]; warning: string }> = {
  ...
  'Balance Scale': {
    ...
    warning: '⚠️ Pre-cut all materials and strings to EQUAL lengths — unequal strings mean one pan always hangs lower and every measurement is wrong!',
    steps: [
      'Pre-cut 1 cardstock base rectangle (~20 × 15 cm) per kid — the stable platform the uprights sit on.',
      'Pre-cut 6 string pieces (15 cm each) per kid — 3 per pan, 2 pans. All strings MUST be the same length.',
      'Pre-cut 2 cardstock circle pans per kid — both pans must be the same size and material.',
      'Have 3 popsicle sticks per kid: 2 uprights (taped side-by-side at base centre, tiny gap between them) and 1 horizontal beam.',
      'Build your own demo balance scale and verify: beam pivots freely in the gap between the uprights, empty pans hang level.',
  ...
  return (
    <div className="min-h-screen bg-orange-50 flex flex-col">
      ...
      <header className="bg-orange-500 text-white px-4 py-4 flex items-center gap-3">
        <button onClick={() => router.push(backHref)} className="text-orange-100 text-2xl">←</button>
